...
class TDS210():
    ...
    def __init__(self):
        ...
        self.index=None
        self.tds_commands=[]
        # Both List (tds_functions And tds_commands) Indices Must Correlate. 
        # At The Present Time, There Are 158 Indices (Functions And Commands) In Both List.
        self.tds_functions=['**** UTILITIES ****','oscope_self_cal','oscope_auto_set','oscope_factory_set','oscope_reset',
    ...
    def oscope_initialize(self):     
        if not Controller_Initialized.get():self.GPIB_Controller_Init()
        try:
    ...
    def send_to_oscope(self,funct):
        ...
        for i, items in enumerate(self.tds_functions):# Find Index Based On Function Text Instead Of Using cbo Index.
        ...
        if len(self.tds_functions)!=len(self.tds_commands):# Make Sure Both List Have The Same Indices.
            msg1='Lists tds_functions And tds_commands Have Different Indices:\n'
        ...
        try:
            if self.tds_commands[self.index]=='CAL:INTERNAL':
                ...
                if self.tds_commands[self.index]=='CURVE?':
                    ...
                    val=self.oscope.query(self.tds_commands[self.index])
                    if ";" in val:
...
# ****************************** Plot 2 ******************************
    ...
    def plot_tds210_curves(exe_style):# Example
        status=GPIB.oscope_initialize()# Initialize Controller And Instrument
        ...
        GPIB.send_to_oscope('oscope_self_cal')
        msg1='Replace The Probes To The Oscilloscope Inputs\n'
        msg2='And Connect The Probe Tips To The Probe Comp\n'
        msg3='5 V Squarewave Output Terminal.\n'
        msg4='When Completed! Press OK Button To Start Measurement.'
        msg=msg1+msg2+msg3+msg4
        messagebox.showinfo('Measure Probe Comp ',msg)
        GPIB.send_to_oscope('set_horiz_main_scale, 500e-6')
        GPIB.send_to_oscope('set_horiz_main_position, 0.0')
        GPIB.send_to_oscope('set_horiz_view, MAIN')
        GPIB.send_to_oscope('set_vert_math, OFF')
        GPIB.send_to_oscope('set_vert_select, CH1, ON') 
        GPIB.send_to_oscope('set_vert_bandwidth, OFF, CH1') 
        GPIB.send_to_oscope('set_vert_coupling, DC, CH1') 
        GPIB.send_to_oscope('set_vert_position, 1, CH1') 
        GPIB.send_to_oscope('set_vert_probe, 10, CH1') 
        GPIB.send_to_oscope('set_vert_scale, 2, CH1') 
        GPIB.send_to_oscope('set_data_encoding, ASCII') # Make Sure Format Is ASCII For Transfer
        GPIB.send_to_oscope('set_data_source, CH1')
        GPIB.send_to_oscope('set_data_start, 1')
        GPIB.send_to_oscope('set_data_stop, 2500')
        GPIB.send_to_oscope('set_data_width, 1')
        GPIB.send_to_oscope('set_trig_mode, NORMAL')
        GPIB.send_to_oscope('set_trig_type, EDGE')# Make Sure Trigger Is OK
        GPIB.send_to_oscope('set_trig_edge_source, CH1')
        GPIB.send_to_oscope('set_trig_edge_coupling, DC')
        GPIB.send_to_oscope('set_trig_50%')
        # Set Scope Measurements For Channel 1
        GPIB.send_to_oscope('set_meas1_source, CH1')
        GPIB.send_to_oscope('set_meas1_type, PK2PK')
        GPIB.send_to_oscope('set_meas2_source,CH1')
        GPIB.send_to_oscope('set_meas2_type, CRMS')
        GPIB.send_to_oscope('set_meas3_source,CH1')
        GPIB.send_to_oscope('set_meas3_type, FREQ')
        GPIB.send_to_oscope('set_meas4_source, CH1')
        GPIB.send_to_oscope('set_meas4_type, PERIOD')
        data_source=GPIB.send_to_oscope('get_data_source')# (CH1,CH2,MATH,REFA,REFB), Waveform To Be Transfered
        num_pts=GPIB.send_to_oscope('get_waveform_num_pts')# Should Not Exceed 2500 (data_stop - data_start)
        data_xincr=GPIB.send_to_oscope('get_waveform_xincr')# Time Increments Per Data Point
        ...
        y_scale=GPIB.send_to_oscope('get_vert_scale,'+data_source)# Get Scale Of Curve Data Waveform
...
# ****************************** Channel 1 ******************************
        ...
        plt1_style='solid'# Set The line2d Style
        plt1_width=1# Set The line2d Width
        xdata=GPIB.send_to_oscope('get_curve_xdata')
        ...
        ydata=GPIB.send_to_oscope('get_curve_ydata')
        ...
        source=GPIB.send_to_oscope('get_data_source')
        meas1=GPIB.send_to_oscope('get_meas1_value')
        meas2=GPIB.send_to_oscope('get_meas2_value')
        meas3=GPIB.send_to_oscope('get_meas3_value')
        meas4=GPIB.send_to_oscope('get_meas4_value')
...
# ****************************** Channel 2 ******************************
        ...
        GPIB.send_to_oscope('set_vert_select, CH2, ON') 
        GPIB.send_to_oscope('set_vert_bandwidth, OFF, CH2') 
        GPIB.send_to_oscope('set_vert_coupling, DC, CH2') 
        GPIB.send_to_oscope('set_vert_position, -3.5, CH2') 
        GPIB.send_to_oscope('set_vert_probe, 10, CH2') 
        GPIB.send_to_oscope('set_vert_scale, 2, CH2') 
        GPIB.send_to_oscope('set_data_source, CH2')
        GPIB.send_to_oscope('set_meas1_source, CH2')
        GPIB.send_to_oscope('set_meas1_type, PK2PK')
        GPIB.send_to_oscope('set_meas2_source,CH2')
        GPIB.send_to_oscope('set_meas2_type, CRMS')
        GPIB.send_to_oscope('set_meas3_source,CH2')
        GPIB.send_to_oscope('set_meas3_type, FREQ')
        GPIB.send_to_oscope('set_meas4_source, CH2')
        GPIB.send_to_oscope('set_meas4_type, PERIOD')
...
if __name__ == "__main__":
    ...
    Controller_Initialized=tk.BooleanVar() # For TDS210 Class
